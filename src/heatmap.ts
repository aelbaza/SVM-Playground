...
Modifications Copyright 2022 Long Nguyen.
...
export class HeatMap {
  ...
  updateBackground(data: number[][], discretize: boolean): void {
    if (!data.length || !data[0].length) throw new Error("Boundary is invalid");
  ...
  private updateCircles(container, points: Example2D[]) {
    ...
    selection
      ...
      .on("mouseenter", (event: Event, d: Example2D) => {
        ...
        if (d.voteCounts.length !== 2)
          throw new Error("Vote counts are not valid");
        ...
        d3.select("#hovercard.ui-nvotes #first-class.value").text(
          d.voteCounts[0]
        );
        d3.select("#hovercard.ui-nvotes #second-class.value").text(
          d.voteCounts[1]
        );
      })
      .on("mouseleave", () => {
